...
export type PackageDuration = { durationId?: number; months: number; bonusMonths: number; baseAmount: number; packagePromotion: number; monthlyPrice: number; originalMonthlyPrice: number; totalPrice: number; discountLabel: string; subtitle: string; badge?: string };
export type GymPackage = {
  id: string; apiId: number; name: string; tier: string; description: string; hero: typeof diamondPackage.hero;
...
export const packages: readonly GymPackage[] = [
  { ...shared, id: 'silver-pass', apiId: 1, name: 'Silver Pass - Khởi Đầu', tier: 'SILVER PASS', description: 'Gói tập cơ bản cho người mới bắt đầu.', durations: durations(490000), availability: 'active', accessHours: { from: '08:00', to: '16:00' }, privileges: [privilege('hours', undefined, 'Tập 08:00 - 16:00'), privilege('gym', undefined, 'Phòng tập tạ máy & cardio'), privilege('locker', 'SMART_LOCKER', 'Tủ đồ cá nhân'), privilege('shower', undefined, 'Phòng tắm')] },
  { ...shared, id: 'gold-vip', apiId: 2, name: 'Gold VIP - Phổ Biến Nhất', tier: 'GOLD VIP', description: 'Tập giờ mở rộng cùng các lớp nhóm và tiện ích phục hồi.', durations: durations(815000), availability: 'active', popular: true, accessHours: { from: '06:00', to: '22:00' }, privileges: [privilege('hours', undefined, 'Tập 06:00 - 22:00'), privilege('group', 'GROUP_X', 'Group-X'), privilege('yoga', 'YOGA', 'Yoga'), privilege('sauna', 'SAUNA', 'Sauna'), privilege('inbody', 'INBODY', 'InBody')] },
  { ...diamondPackage, apiId: 3, durations: [{ months: 1, bonusMonths: 0, baseAmount: 1850000, packagePromotion: 0, monthlyPrice: 1850000, originalMonthlyPrice: 1850000, totalPrice: 1850000, discountLabel: 'Giá chuẩn', subtitle: 'Linh hoạt' }, ...diamondDurations], availability: 'active', privileges: [...diamondPackage.privileges.map(p => ({ ...p, code: ({ clubs: 'CHECKIN_24_7', pt: 'PT_SESSION', inbody: 'INBODY', companion: 'GUEST_PASS', sauna: 'SAUNA', daily: 'DETOX', shop: 'PRO_SHOP_DISCOUNT' } as Record<string, PrivilegeCode>)[p.id], included: p.id === 'pt' ? 3 : p.id === 'companion' ? 4 : undefined, resetPeriod: p.id === 'companion' ? 'month' as const : 'membership' as const })), privilege('group', 'GROUP_X', 'Group-X & Yoga'), privilege('yoga', 'YOGA', 'Yoga'), privilege('locker', 'SMART_LOCKER', 'Smart Locker')] },
  { ...shared, id: 'student-pass', apiId: 4, name: 'Gói Student / HSSV', tier: 'STUDENT', description: 'Ưu đãi dành cho học sinh, sinh viên có thẻ còn hiệu lực.', durations: durations(350000), availability: 'active', requiresStudentVerification: true, accessHours: { from: '08:00', to: '17:00' }, privileges: [privilege('hours', undefined, 'Tập 08:00 - 17:00'), privilege('gym', undefined, 'Phòng tập tạ máy & cardio'), privilege('locker', 'SMART_LOCKER', 'Tủ đồ cá nhân')] },
];
export const getPackageById = (id: string) => packages.find(item => item.id === id) ?? null;
export const getPackagePresentation = (apiId: number): GymPackage => packages.find(item => item.apiId === apiId) ?? {
  ...shared,
  id: String(apiId),
  apiId,
  name: '',

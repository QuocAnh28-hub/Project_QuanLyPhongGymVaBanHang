    ...
    <SafeAreaView edges={['top']} style={styles.safeArea}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <>
          ...
          <View style={styles.passCard}><Text style={styles.passKicker}>QA-GYM PREMIUM</Text><Text style={styles.passName}>{activeMembership?.packageName.toUpperCase() ?? 'CHƯA CÓ GÓI TẬP'}</Text>{activeMembership ? <View style={styles.passInfo}><Text style={styles.passLabel}>MÃ ĐĂNG KÝ{`\n`}{activeMembership.id}</Text><Text style={styles.passLabel}>HẠN SỬ DỤNG{`\n`}{activeMembership.expiryDate}</Text></View> : <Text style={styles.passMutedText}>Chỉ gói đã thanh toán và kích hoạt mới hiển thị tại đây.</Text>}</View>
          {pendingMembership ? <><View style={styles.sectionHeading}><Text style={styles.sectionTitle}>ĐƠN ĐĂNG KÝ ĐANG CHỜ</Text><Text style={styles.sectionAction}>CHỜ THANH TOÁN</Text></View><Pressable style={styles.passCard} onPress={() => router.push({ pathname: '/package-payment', params: { orderId: pendingMembership.id } })}><Text style={styles.passKicker}>{pendingMembership.id}</Text><Text style={styles.passName}>{pendingMembership.packageName.toUpperCase()}</Text><Text style={styles.passBenefits}>Chạm để xem hướng dẫn thanh toán · {formatVND(pendingMembership.totalPrice)}</Text></Pressable></> : null}

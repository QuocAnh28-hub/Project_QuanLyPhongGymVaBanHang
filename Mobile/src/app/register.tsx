...
export default function Register() {
  ...
  return (
    <AuthScreen>
      <View style={s.promo}>
        <Text style={s.promoBadge}>ϟ ĐẶC QUYỀN HỘI VIÊN MỚI</Text>
        <Text style={s.promoTitle}>
          Gia nhập <Text style={{ color: C.lime }}>QA-Gym</Text>
        </Text>
        <Text style={s.promoCopy}>
          Tạo tài khoản hội viên để nhận ngay 1 buổi tập 1-1 cùng HLV & phân
          tích chỉ số InBody 770 miễn phí!
        </Text>
          ...
          {[
            ...
            <View style={s.benefit} key={title}>
              <Ionicons
                ...
                color={C.mint}
              />
              <Text style={s.benefitText}>{title}</Text>
            </View>
          ))}
        </View>
      </View>
          ...
          {[1, 2, 3].map((n) => (
            <View
              ...
              style={[
                s.bar,
                strength >= n && {
                  backgroundColor: strength === 1 ? C.error : C.lime,
                },
              ]}
            />
          ))}
      ...
      <View style={s.goals}>
        {goals.map((goal) => {
          ...
          return (
            ...
            >
              <View style={[s.goalIcon, selected && s.goalIconSelected]}>
                <Ionicons
                  name={goal.icon}
                  size={19}
              ...
              <View style={{ flex: 1 }}>
                <Text style={[s.goalTitle, selected && { color: C.lime }]}>
                  {goal.title}
                </Text>
                <Text style={s.goalDetail}>{goal.detail}</Text>
              </View>
            </Pressable>
          );
        })}
      ...
      >
        <View style={[s.check, terms && { backgroundColor: C.lime }]}>
      ...
      <View style={s.community}>
        <Ionicons name="trophy-outline" size={26} color={C.lime} />
        <View>
          <Text style={s.communityTitle}>Hơn 12,400+ gymer đã tham gia</Text>
          <Text style={s.communityCopy}>
            Cộng đồng tập luyện bứt phá giới hạn số 1
          </Text>
        </View>
      </View>
...
const s = StyleSheet.create({
  ...
  promoTitle: { color: C.text, fontSize: 26, fontWeight: "800" },
  promoCopy: { color: C.muted, fontSize: 13, lineHeight: 20, marginTop: 9 },
  ...
  benefitText: { color: C.text, fontSize: 10, fontWeight: "700", flex: 1 },
  ...
  hint: { color: C.lime, fontSize: 11, fontWeight: "700" },
  ...
  goalTitle: { color: C.text, fontSize: 11, fontWeight: "700" },
  ...
  termsText: { color: C.muted, flex: 1, fontSize: 12, lineHeight: 19 },
  ...
  communityTitle: { color: C.text, fontSize: 12, fontWeight: "700" },
  communityCopy: { color: C.muted, fontSize: 11, marginTop: 3 },
});

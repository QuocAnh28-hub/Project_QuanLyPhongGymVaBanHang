...
export default function PackageEnrollmentScreen() {
  ...
  const item = getPackageById(typeof params.id === "string" ? params.id : "");
    ...
    try {
      ...
      const activationDate = renewal
        ? renewal.expiryDate > localDate(today)
          ...
          : localDate(today)
        : form.activationDate;
      const canonical = calculatePrice(selectedOption, appliedVoucher);
      const order = await createEnrollment({
        userId: currentUser.email,
        packageId: selectedItem.id,
        packageName: selectedItem.name,
        durationMonths: selectedOption.months,
        bonusMonths: selectedOption.bonusMonths,
        ...
        renewalOf: renewal?.id,
        termsAcceptedAt: new Date().toISOString(),
        gifts:
          selectedItem.id === "diamond-all-access"
            ? enrollmentGifts.map(({ id, quantity, name, detail }) => ({
                id,
                quantity,
                name,
                detail,
      ...
      >
        ...
        >
          ...
          <Card>
            ...
            <View style={s.totalRow}>
              ...
              <Text style={s.total}>{formatVND(pricing.finalAmount)}</Text>
            </View>
          </Card>
          <Pressable
            style={s.terms}
        ...
        >
          <View>
            ...
            <Text style={s.bottomPrice}>{formatVND(pricing.finalAmount)}</Text>
          </View>
          <Pressable
            ...
            style={[
              s.primary,
              s.submit,
              (isSubmitting || !termsAccepted) && s.disabled,
            ]}
          >
...
const s = StyleSheet.create({
  ...
  stepNumber: { color: C.muted, fontSize: 9, fontWeight: "800" },
  stepNumberActive: { color: C.surfaceLowest },
  stepLabel: { color: C.muted, fontSize: 8, fontWeight: "800", marginTop: 5 },
  ...
  packageName: { color: C.text, fontSize: 17, fontWeight: "800", marginTop: 5 },
  muted: { color: C.muted, fontSize: 11, flex: 1 },
  ...
  strike: { color: C.muted, fontSize: 10, textDecorationLine: "line-through" },
  price: { color: C.lime, fontSize: 21, fontWeight: "900" },
  save: {
  ...
  giftTitle: { color: C.mint, fontSize: 8, fontWeight: "900" },
  gift: {
    flexDirection: "row",
    alignItems: "center",
    gap: 9,
  ...
  giftName: { color: C.text, fontSize: 10, fontWeight: "600" },
  small: { color: C.muted, fontSize: 9, lineHeight: 13, flexShrink: 1 },
  sectionHead: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
  ...
  label: { color: C.muted, fontSize: 8, fontWeight: "800", letterSpacing: 0.5 },
  field: { gap: 5 },
  inputWrap: {
    minHeight: 48,
    flexDirection: "row",
  ...
  inputText: { flex: 1, color: C.text, fontSize: 12, paddingVertical: 10 },
  selectText: { flex: 1, color: C.text, fontSize: 12 },
  hint: { color: C.muted, fontSize: 9, lineHeight: 13, marginTop: -5 },
  error: { color: C.error, fontSize: 10, lineHeight: 14 },
  ...
  primaryText: { color: C.surfaceLowest, fontSize: 10, fontWeight: "900" },
  limeLabel: { color: C.lime, fontSize: 8, fontWeight: "800" },
  applied: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  ...
  appliedText: { color: C.text, fontSize: 10 },
  secure: { color: C.mint, fontSize: 8, fontWeight: "800" },
  method: {
    flexDirection: "row",
    alignItems: "center",
    gap: 9,
  ...
  costValue: { color: C.text, fontSize: 11, fontWeight: "800" },
  ...
  total: { color: C.lime, fontSize: 22, fontWeight: "900", flexShrink: 0 },
  terms: {
    flexDirection: "row",
  ...
  termsText: { color: C.muted, flex: 1, fontSize: 10, lineHeight: 15 },
  ...
  bottomPrice: { color: C.lime, fontSize: 18, fontWeight: "900" },
  primary: {
    minHeight: 48,
    backgroundColor: C.lime,
    borderRadius: 10,
